    ...
    <div className="space-y-8">{items.map((item, i) => (
      <motion.div key={item.slug} initial={{opacity:0,x:-15}} whileInView={{opacity:1,x:0}} viewport={{once:true}} transition={{duration:0.4,delay:i*0.08}} className="relative pl-8">
        ...
        <div className="card p-5">
          {/* Thumbnail */}
          {item.meta.thumbnail && (
            <div className="relative w-full h-40 rounded-md overflow-hidden mb-4 bg-gray-50">
              <Image src={item.meta.thumbnail} alt={item.meta.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 640px" />
            </div>
          )}
          <div className="flex flex-wrap items-center gap-3 mb-1.5">
            ...
            {item.meta.category && (
              <span className={`text-[10px] uppercase tracking-wider font-medium px-2 py-0.5 rounded ${
                item.meta.category === "work" ? "bg-navy/10 text-navy" :
                item.meta.category === "organization" ? "bg-gold/10 text-gold-dark" :
